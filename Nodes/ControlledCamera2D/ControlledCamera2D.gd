...
func get_target_pos() -> Vector2:
	...
	var ret: Vector2 = follow_node.global_position if follow_node_pos else follow_pos
...
#	var b: float = limit_bottom - (view_size.y / 2)
	...
	ret.x = max(min(ret.x, limit_right - (view_size.x / 2)), limit_left + (view_size.x / 2))
	ret.y = max(min(ret.y, limit_bottom - (view_size.y / 2)), limit_top + (view_size.y / 2))
...
#	var smooth_y: bool = target_pos.y - global_position.y > 1 or true
	...
	var target_pos: Vector2 = get_target_pos()
...
#	camera.global_position.y = lerp(camera.global_position.y, target_pos.y, delta*y_smoothing)
	if lerp_mode:
		...
		var do_lerp: bool = (camera.global_position - target_pos).length() >= _movement_threshold
		
		if x_smoothing > 0.0:
			if do_lerp:
				camera.global_position.x = lerp(camera.global_position.x, target_pos.x, x_smoothing*delta)
		else:
			camera.global_position.x = target_pos.x
		...
		if not do_lerp and prev_do_lerp:
			emit_signal("STOPPED")
		
		prev_do_lerp = do_lerp
...
func _interpolate_limit(limit_key: String, axis: String, target_value: float):
	...
	while true:
		var delta: float = yield(get_tree(), "idle_frame")
...
func set_dim_layer(layer: int, offset: int = 0):
	dim_layer = layer
	Game.set_node_layer(dimColorRectContainer, layer, offset)
